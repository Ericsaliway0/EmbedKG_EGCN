...
def load_graph_data(file_path):
    ...
    embedding_size = 256  # Ensure this matches your actual embedding dimension
    ...
    # Ensure all edges have the 'type' attribute
    for u, v, data in G_nx.edges(data=True):
        if 'type' not in data:
            data['type'] = -1  # Default value for missing 'type'

    # Convert the NetworkX graph to a DGL graph
    G_dgl = dgl.from_networkx(G_nx, edge_attrs=['type'])

    # Extract node features, ensuring all are 256-dimensional
    node_features = []
    for node in G_nx.nodes():
        if 'embedding' in G_nx.nodes[node] and len(G_nx.nodes[node]['embedding']) == embedding_size:
            node_features.append(G_nx.nodes[node]['embedding'])
        else:
            node_features.append([0] * embedding_size)  # Zero vector of size 256
...
print("Graph successfully loaded with", node_features.shape[0], "nodes and embedding size:", node_features.shape[1])
'''
