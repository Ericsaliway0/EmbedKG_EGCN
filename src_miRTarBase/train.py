...
from .models import EGCN, GAT, GraphSAGE, ChebNet, ChebNetII, GIN, GCN, MLPPredictor, FocalLoss
from src.utils import (choose_model)
#, plot_roc_curve, plot_pr_curve, load_graph_data,load_oncokb_genes, plot_and_analyze, save_and_plot_results)
from .utils import (plot_training_validation_metrics, plot_roc_pr_curves, find_evidence, compute_hits_k, compute_auc, compute_f1, compute_focalloss,
                    compute_accuracy, compute_precision, compute_recall, compute_map, save_overall_metrics,
...
def train_and_evaluate(args, G_dgl, node_features, node_id_to_name):
    ...
    # Choose the model
    model = choose_model(args.model_type, args.in_feats, args.hidden_feats, args.out_feats)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(eids)):
        ...
        train_g = dgl.remove_edges(G_dgl, test_idx.tolist() + val_idx.tolist()).to(args.device)

        def create_graph(u, v, num_nodes):
            return dgl.graph((u, v), num_nodes=num_nodes).to(args.device)
        ...
        ##for e in range(args.epochs):
            ...
            model.train()
            h = model(train_g, train_g.ndata['feat']).to(args.device)
            ...
            if e % 5 == 0:
                print(f'Fold {fold + 1} | Epoch {e} | Loss: {loss.item():.4f} | Train Accuracy: {train_acc:.4f} | Val Accuracy: {val_acc:.4f}')
        
        
            epoch_end_time = time.time()
            epoch_duration = epoch_end_time - epoch_start_time
            ## {epoch_duration:.4f} seconds")

        total_time = time.time() - start_time
        average_time_per_epoch = total_time / args.epochs
        print(f"Average training time per epoch: {average_time_per_epoch:.4f} seconds")
        ...
        with torch.no_grad():
            ...
            output_path_test_metrics = f'{args.model_type}_{args.net_type}_test_curve_5_fold_drop{args.attn_drop}_head{args.num_heads}_lr{args.lr}_lay{args.num_layers}_input{args.input_size}_dim{args.out_feats}_epoch{args.epochs}.txt'
        ...
        # Get CPU and GPU memory usage
        cpu_usage = process.memory_info().rss / (1024 ** 2)  # Convert to MB
        gpu_usage = torch.cuda.memory_allocated(args.device) / (1024 ** 2) if torch.cuda.is_available() else 0

        print(f"Test Loss: {test_focal_loss:.4f}, Training Time: {total_time:.4f}s, Average Time: {average_time_per_epoch:.4f}s, CPU Usage: {cpu_usage:.2f} MB, GPU Usage: {gpu_usage:.2f} MB")

        # Save overall metrics
        save_overall_metrics(total_time, average_time_per_epoch, cpu_usage, gpu_usage, args, output_path)

        '''plot_roc_curves(fold_results, output_path_cross_roc)
        plot_pr_curves(fold_results, output_path_cross_pr)'''
        output_path_roc_pr = os.path.join(output_path, f'{args.model_type}_{args.net_type}_roc_pr_curve_5_fold_drop{args.attn_drop}_head{args.num_heads}_lr{args.lr}_lay{args.num_layers}_input{args.input_size}_dim{args.out_feats}_epoch{args.epochs}.png')
        ...
        filename_u = f'{args.model_type}_{args.net_type}_top_scores_u_lr{args.lr}_lay{args.num_layers}_input{args.input_size}_dim{args.out_feats}_epoch{args.epochs}.csv'
        save_dataframe(df_top_u, filename_u)
        filename_v = f'{args.model_type}_{args.net_type}_top_scores_v_lr{args.lr}_lay{args.num_layers}_input{args.input_size}_dim{args.out_feats}_epoch{args.epochs}.csv'
        save_dataframe(df_top_v, filename_v)

        # Load reference data
        '''file2 = 'data/miRNet-mir-gene-hsa-tarbase.csv'
        ...
        filename_breast_cancer = f'{args.model_type}_{args.net_type}_top_breast_cancer_lr{args.lr}_lay{args.num_layers}_input{args.input_size}_dim{args.out_feats}_epoch{args.epochs}.csv'
